...
from physiopro.network.divfree import CurlVectorField, check_divergence_free, DivergenceFreeODEFunc  # Use the divfree module
...
parser.add_argument('--lr', type=float, default=0.003)  # Reduced from 0.01 for curl fields
...
parser.add_argument('--atol', type=float, default=0.001)  # Tighter tolerance for better behavior
parser.add_argument('--rtol', type=float, default=0.001)  # Tighter tolerance for better behavior
...
class NeuralODE(nn.Module):
    def __init__(self, obs_dim, device, batch_size=100):  # Reduced from 200 for autograd-Jacobians
        super(NeuralODE, self).__init__()
        self.latent_dim = 8
        # Use DivergenceFreeODEFunc wrapper for proper time handling
        self.func = DivergenceFreeODEFunc(dim=self.latent_dim, hidden_dims=[16]).to(device)
...
class ContiFormer(nn.Module):
    def __init__(self, obs_dim, device, batch_size=64, divergence_free: Optional[bool] = True):
...
if __name__ == '__main__':
    ...
    if args.model_name == 'Neural_ODE':
        ...
        log.info("Checking divergence-free property of Neural ODE...")
        check_divergence_free(model.func, dims=model.latent_dim, n=64)
    elif args.model_name == 'Contiformer':
        model = ContiFormer(obs_dim, device)
        # Check divergence-free property of ContiFormer ODEs
        log.info("Checking divergence-free property of ContiFormer ODEs...")
        if hasattr(model.encoder, 'slf_attn'):
            layer = model.encoder
            if hasattr(layer.slf_attn.w_ks, 'ode_func'):
                log.info("  Keys ODE:")
                check_divergence_free(layer.slf_attn.w_ks.ode_func, dims=16, n=64)
            if hasattr(layer.slf_attn.w_vs, 'ode_func'):
                log.info("  Values ODE:")
                check_divergence_free(layer.slf_attn.w_vs.ode_func, dims=16, n=64)
    else:
    ...
    for itr in range(st + 1, args.niters + 1):
        ...
        # Periodically check divergence-free property
        if itr % 100 == 0 and args.model_name == 'Neural_ODE':
            log.info(f"Checking divergence at iteration {itr}...")
            check_divergence_free(model.func, dims=model.latent_dim, n=64)
