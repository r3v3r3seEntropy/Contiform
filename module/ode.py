...
# Import divergence-free modules
try:
    from physiopro.network.divfree import DivergenceFreeODEFunc
except ImportError:
    DivergenceFreeODEFunc = None
...
def build_fc_odefunc(dim=2, hidden_dims=[64, 64, 64], out_dim=None, nonzero_dim=None, actfn="softplus",
                     layer_type="concat",
                     zero_init=True, actfirst=False, divergence_free=False):
    """
    Modified to support divergence-free vector fields when divergence_free=True
    """
    if divergence_free and DivergenceFreeODEFunc is not None:
        # Use divergence-free ODE function
        return DivergenceFreeODEFunc(dim=dim, hidden_dims=hidden_dims)
